                    ...
                    <v-card>
                        <v-container>   
                            <v-form @submit.prevent="addEvent">
                                ...
                                <v-text-field v-model="start" type="date" label="Day (required)">
                                </v-text-field>
                                <v-text-field v-model="startTime" type="text" label="Start time (0:00-24:00)">
                                </v-text-field>
                                <v-text-field v-model="endTime" type="text" label="End time (0:00-24:00)">
                                </v-text-field>
                                <v-text-field v-model="color" type="color" label="Color (click to open menu)">
                                </v-text-field>
                                <v-select
                                        :items="employees"
                                        label="Employee"
                                        v-model = "shiftTo"
                                ></v-select>
                                <v-btn type="submit" color="primary" class="mr-4" @click="dialog=false">
                                   Add Shift
                                </v-btn>
                            </v-form>
                        </v-container>
      ...
      <v-sheet height="64">
        ...
        >
          ...
          <v-btn
            color="primary"
            class="mr-4"
            @click="dialog=true; getEmployees();"
            dark
          >
            Add Shift
          </v-btn>
...
export default {
    ...
    methods: {
            async setCode(){
                ...
                document.getElementById("code-text-field").value = 'Employer Code: ' + code;
            },
            async getEmployees(){
                var user = firebase_app.auth().currentUser;
                var doc = db.collection('users').where('email','==',user.email);
            ...
            async getEvents(){
                var user = firebase_app.auth().currentUser;
                let snapshot = await db.collection("events").where('employer','==',user.email).get();
                let events = []
            ...
            async addEvent(){
                if(this.name && this.start && this.shiftTo && this.startTime && this.endTime){
                    var user = firebase_app.auth().currentUser;
                    ...
                    var doc = db.collection('users').where('email','==',user.email);
                    var code = (await doc.get()).docs[0]._delegate._document.data.value.mapValue.fields.code.stringValue;
                    var doc2 = db.collection('users').where('firstname','==',toFirstName).where('lastname','==',toLastName).where('code','==',code);
                    var toEmail = (await doc2.get()).docs[0]._delegate._document.data.value.mapValue.fields.email.stringValue;
